...
const buildQRData = (cropPda, crop, farmer) => JSON.stringify({
  agrichain: true,
  status:      crop.status === 4 ? "APPROVED" : "REJECTED",
  cropId:      crop.cropId,
  cropPda,
  farmer:      farmer?.name || "Unknown",
  mobile:      farmer?.mobile || "",
  district:    DISTRICT_MAP[farmer?.district] || "",
  crop:        CROP_MAP[crop.cropType] || crop.cropType,
  finalWeight: crop.finalWeight?.toString() + " kg",
  ...
  pkgType:     PKG_TYPES[crop.packageType] || "",
  material:    crop.packagingMaterial,
  testResults: crop.testResults,
...
const drawQR = async (canvasRef, data) => {
  ...
  try {
    await window.QRCode.toCanvas(canvasRef.current, data, {
      width:220, margin:2, color:{ dark:"#0a0f07", light:"#6dce3f" },
    });
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{fontSize:13,fontWeight:700,letterSpacing:"2px",textTransform:"uppercase",
          color:approved?"#6dce3f":"#e05c5c",marginBottom:6}}>
          {approved ? "✅ Crop Approved" : "❌ Crop Rejected"}
        </div>
        <div style={{fontSize:11,color:"rgba(240,247,235,0.4)",marginBottom:20}}>
          {approved ? "QR for manufacturer handoff" : "QR for collector re-routing"}
        </div>
        <div style={{display:"inline-block",padding:8,borderRadius:12,
        ...
        <div style={{display:"flex",gap:8,justifyContent:"center"}}>
          <button onClick={download} style={{
            background:"linear-gradient(135deg,#3da81c,#6dce3f)",border:"none",
            borderRadius:9,color:"#050a03",fontWeight:700,fontSize:12,
            padding:"9px 18px",cursor:"pointer",fontFamily:"inherit",
            boxShadow:"0 3px 12px rgba(109,206,63,0.3)",
          }}>⬇ Download QR</button>
          <button onClick={onClose} style={{
            background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",
            borderRadius:9,color:"rgba(240,247,235,0.8)",fontSize:12,
            padding:"9px 18px",cursor:"pointer",fontFamily:"inherit",
          }}>Close</button>
        </div>
        <div style={{marginTop:14,fontSize:10,color:"rgba(240,247,235,0.35)"}}>
          {approved ? "Show to manufacturer for final handoff" : "Show to collector — crop needs re-inspection"}
        </div>
      </div>
    </div>
  );
...
export default function Lab() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const initD = {};
      myCrops.forEach(({publicKey:pk}) => { if (decisions[pk.toString()] === undefined) initD[pk.toString()] = true; });
      ...
      <div className="page">
        ...
        {!connected ? (
          <div className="card" style={{maxWidth:400}}>
            <div className="no-wallet"><div className="no-wallet-icon">🔒</div><p>Connect the Lab Phantom wallet</p></div>
          </div>
        ) : (
